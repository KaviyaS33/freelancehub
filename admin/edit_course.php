...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$title || !$youtube_url) {
        ...
        $pdo->prepare("UPDATE courses SET title=?, category=?, level=?, youtube_url=?, duration_hrs=?, description=?, project_task=? WHERE id=?")
            ->execute([$title, $category, $level, $youtube_url, $duration_hrs, $description, $project_task, $id]);
...
<head>
<meta charset="UTF-8">
<title>Edit Course – FreelanceHub</title>
<style>
...
.btn{padding:13px 26px;background:#7c3aed;color:#fff;border:none;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer}
...
<div class="main">
    <a href="view_courses.php" class="back-btn">← Back</a>
    <h1>Edit Course</h1>
    ...
    <div class="card">
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="url" name="youtube_url" value="<?= htmlspecialchars($course['youtube_url']) ?>" required>
            </div>
            <div class="form-group">
                <label>Duration (hrs)</label>
                <input type="number" name="duration_hrs" value="<?= $course['duration_hrs'] ?>">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea name="project_task"><?= htmlspecialchars($course['project_task']) ?></textarea>
            </div>
